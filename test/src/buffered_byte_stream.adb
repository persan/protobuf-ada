...
package body Buffered_Byte_Stream is
  ----------
  -- Read --
  ----------
  ...
  begin
    if not (The_Byte_Buffer.Count > 0) then
      Last := Item'First - 1;
      return;
    end if;
    ...
    begin
      ...
      Last := Ada.Streams.Stream_Element_Offset (Buffer_Index);
    end;
  ...
  is
    use type Ada.Containers.Count_Type;
    use type Ada.Streams.Stream_Element_Offset;
    Item_Size : Ada.Streams.Stream_Element_Offset := Item'Last - Item'First + 1;
  begin
  ...
  is
    ...
    Length             : Ada.Streams.Stream_Element_Offset := Ada.Streams.Stream_Element_Offset (Stream.Byte_Stream_Buffer.Count);
